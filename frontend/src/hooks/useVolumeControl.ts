...
export const useVolumeControl = (initialApplications: Application[] = []) => {
  ...
  const updateAppVolume = useCallback(
    async (appName: string, volume: number) => {
      const controller = new AbortController();
  ...
  const handleVolumeChange = useCallback(
    (appName: string, newVolume: number, isFinalUpdate = false) => {
      ...
      setApplications((prev) =>
        prev.map((app) => {
          ...
          const isMuted =
            newVolume === 0 ? true : app.isMuted ? false : app.isMuted;
  ...
  const handleVolumeChangeEnd = useCallback(
    (appName: string, newVolume: number) => {
  ...
  const handleWebSocketVolumeChange = useCallback(
    ({
      appName,
      volume,
      action,
      ...
      const hasInFlightRequest = Boolean(
        volumeRequestControllers.current[appName]
      );
      ...
      if (
        ...
        typeof lastRequested === "number" &&
        lastRequested !== volume
      ) {
