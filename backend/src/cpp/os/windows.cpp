...
namespace nodeAudioVolumeMixer {
    ...
    // Get system volume (0-1)
    Napi::Number GetMasterVolumeLevelScalar(const Napi::CallbackInfo& info) {
        Napi::Env env = info.Env();
        float volume = 1.0f; // Default to 100%
        // Implementation to get system volume
        return Napi::Number::New(env, volume);
    }
    
    // Set system volume (0-1)
    void SetMasterVolumeLevelScalar(const Napi::CallbackInfo& info) {
        Napi::Env env = info.Env();
    ...
    Napi::Boolean IsMasterMuted(const Napi::CallbackInfo& info) {
        Napi::Env env = info.Env();
        bool isMuted = false;
        // Implementation to check mute status
        return Napi::Boolean::New(env, isMuted);
    }
    
    // Toggle system mute
    void MuteMaster(const Napi::CallbackInfo& info) {
        Napi::Env env = info.Env();
        ...
        // Implementation to set mute status
    }
